...
<form action="{{ route('result-update', $result->id) }}" method="POST" style="max-width: 700px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;">
    ...
    @foreach ($result->calculateMarks as $mark) 
    
        <div class="form-group" style="flex: 1;">
            <label for="course" style="font-weight: bold;">Course</label>
            <input type="text" class="form-control" name="course[]" value="{{ old('course', $mark->course[0]->courseName) }}" placeholder="Course">
        </div>

        <div class="form-group" style="flex: 1;">
            <label for="marks" style="font-weight: bold;">Marks</label>
            <input type="number" class="form-control" name="marks[]" value="{{ old('marks', $mark->marks) }}" placeholder="Marks">
        </div>
    @endforeach
